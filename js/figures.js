...
var Figure = function(figNum) {
	...
	this.htmlString = "<div ng-app='assembly' ng-controller='assemblycontroller"+this.figNum+"' class='container'>\
	<div class ='row'>\
	<div id='"+this.figNum+"editor_button_group' class='col-lg-4 col-md-4 btn-group-horizontal'\
...
<div class='row'>\
\
<div class='col-lg-8 col-md-8' id='"+this.figNum+"table' style='overflow-y: scroll; border: 1px solid black; height: 200px' id='fig113'>\
<table id='"+this.figNum+"program'>"
			+ this.figCode
			+ "</table>\
</div>\
\
			...
			<div class='container'>\
				<div class='row'>\
					<div class='col-lg-4 col-lg-offset-1 col-md-4 col-md-offset-1'>\
						<div id='"+this.figNum+"table' style='width: 90px'>\
							<table id='"+this.figNum+"variables' class='table table-bordered' ng-repeat='var in variables'>\
...
\
						...
						<div id='"+this.figNum+"table' style='overflow-y: scroll; border: 1px solid black; height: 200px; width: 350px'>\
							<table id='"+this.figNum+"commands'>\
							</table>\
						</div>\
					</div>\
\
					...
					<div class='col-lg-4 col-md-4'>\
						<div id='"+this.figNum+"editor_button_group' class='col-lg-4 col-md-4 btn-group-vertical'>\
...
\
						...
						<div id='"+this.figNum+"table' style='overflow-y: scroll; border: 1px solid black; height: 300px; width: 200px'>\
...
\
	...
	var divName = document.getElementById("fig" + figNum + "Div");
	divName.innerHTML = this.htmlString;
	var assemblyName = this.figNum + 'assembly';
	var assemblerName = this.figNum + 'assembler';
	var tabsstuff = angular
			...
			.provider(
					...
					function() {
						...
						var assembler = function(tableName, varTable,
								figureMode) {
							...
							// A flag indicating whether the program has been
							// run before
							// Primarily used for checking if values should be
							// reset
							this.done = false;
							...
							// Flag for Negative
							// Set whenever a register has stored a negative
							// value
							this.negativeFlag = 0;
							...
							// Alerts the controller that the program has
							// finished
							this.stop = false;
							...
							this.register = [ [ "Reg0", 0, false ], // Reg0
							[ "Reg1", 0, false ], // Reg1
							[ "Reg2", 0, false ], // Reg2
							[ "Reg3", 0, false ], // Reg3
							[ "Reg4", 0, false ], // Reg4
							...
							// Performs the size checking of a register at index
							// to ensure 16bit functionality
							...
							this.checkRegister = function(index) {
								...
								if (regValue > 32768) {
									this.overflow = 1;
									var hex = this
											.decimalToHex(this.register[index][1]);
									var length = hex.length;
									var value = "";
									for (var i = 8; i >= 1; i--) {
										value = value + hex[length - i];
									}
									this.register[index][1] = parseInt(value,
											16);
								}
								if (regValue < -32768) {
									var hex = this
							...
							// Goes through and checks through the table for
							// changes
							...
							this.init = function() {
								...
								while (progLine < table.rows.length) {
									...
									case ".Block": // .Block before program
										...
										// Store variable for display
										// Note: May behave strangely with
										// multiple Words
										this.variables[index] = [
												table.rows[progLine].cells[this.labelNum].firstChild.nodeValue,
												0 ];
									...
									case "Load": // 0001b Load
										...
										// Find correct memory location via
										// label
									...
									case "Store": // 0010b Store
										...
										// Convert decimal memory location to
										// hex
										var hex = parseInt(arg2, 10);
										hex = this.decimalToHex(hex, 2);
										// Store in memory and update program
										// counter
										this.memory[memLine++] = [ 2, arg1,
												hex[0], hex[1] ];
										break;
								...
								// Iterate over registers to see which ones are
								// used
							...
							this.sub = function(reg1, reg2, reg3) {
								...
								console.log("Subtract "
										+ this.register[reg2][0] + " and "
										+ this.register[reg3][0]);
								console.log(this.register[reg1][0] + "= "
										+ this.register[reg1][1]);
							};

							// Store the data in reg into memory at location
							// pointed to by value1 and value2
							...
							// Performs Binary And on two registers and stores
							// into the first
							...
							// Performs Binary Or on two registers and stores
							// into the first
							...
							// Performs Binary Not on a register and stores it
							// into another
							...
							this.load = function(reg, mem1, mem2) {
								...
								this.register[reg][1] = parseInt(num[0]
										+ num[1] + num[2] + num[3], 16);
								// Update Variable array
								for (var i = 0; i < this.variables.length; i++) {
							...
							this.loadImm = function(reg, value1, value2) {
								...
								this.register[reg][1] = parseInt(value1
										+ value2, 16);
								// Update Variable array
								for (var i = 0; i < this.variables.length; i++) {
									if (this.variables[i][0] == this.register[reg][0]) {
							...
							// Load into a register the value in memory pointed
							// to by another register
							...
							// Sets program counter to be equal to the given
							// memory address
							this.jump = function(addr1, addr2) {
								this.programCounter = parseInt(addr1 + addr2,
										16);
							...
							this.halt = function() {
								...
								for (var i = 0; i < numCells; i++) { // iterate
									...
									// the
									// cells
									table.rows[this.programCounter].cells[i].style.color = '#000000'; // highlight
									// all
									// cells
									// black
								}
							...
							this.eval = function(line) {
								...
								case 13: // 1101b Branch
									...
									break;
								case 14: // 1110b Jump
									console.log(this.memory[line][2] + ""
											+ this.memory[line][3]);
								...
								if (!this.stop) {
									...
									for (var i = 0; i < numCells; i++) { // iterate
										...
										table.rows[this.previousCounter
												- this.offSet].cells[i].style.color = '#000000'; // highlight
										// all
										// cells
										// black
							...
							// First checks if the code has recently been
							// edited.
							this.run = function() {
								if (this.edited) {
									this.init();
							...
							// Resets the program counter and restores program
							// to original state
							this.reset = function() {
...
console.log("got here");
	var name = 'assemblycontroller' + this.figNum;
	tabsstuff.controller('assemblycontroller', function($scope, assembler, $interval) {
		...
		var tableName = this.figNum + "program";
		var varTable = this.figNum + "variables";
		var bool = false;
		...
		$scope.architecture = function() {
			...
			$scope.variables = [ {
				title : variables[0],
				titletoo : variables[5]
			}, {
				title : variables[1],
				titletoo : variables[6]
			}, {
				title : variables[2],
				titletoo : variables[7]
			}, {
				title : variables[3],
				titletoo : variables[8]
			}, {
				title : variables[4],
				titletoo : variables[9]
			}, ];
			var register = $scope.assembler.register;
			$scope.register = [ {
				content : register[0][0],
				value : register[0][1]
			}, {
				content : register[1][0],
				value : register[1][1]
			}, {
				content : register[2][0],
				value : register[2][1]
			}, {
				content : register[3][0],
				value : register[3][1]
			}, {
				content : register[4][0],
				value : register[4][1]
			}, {
				content : register[5][0],
				value : register[5][1]
			}, {
				content : register[6][0],
				value : register[6][1]
			}, {
				content : register[7][0],
				value : register[7][1]
			}, {
				content : register[8][0],
				value : register[8][1]
			}, {
				content : register[9][0],
				value : register[9][1]
			}, {
				content : register[10][0],
				value : register[10][1]
			}, {
				content : register[11][0],
				value : register[11][1]
			}, {
				content : register[12][0],
				value : register[12][1]
			}, {
				content : register[13][0],
				value : register[13][1]
			}, {
				content : register[14][0],
				value : register[14][1]
			}, {
				content : register[15][0],
				value : register[15][1]
			}, ];
